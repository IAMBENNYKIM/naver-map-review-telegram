...
def _run_pipeline(
    ...
    if action == "update":
        last_place_id = dynamo_writer.get_last_place_id(chat_id)
        ...
        place_info = naver_review_collector.resolve_place(naver_url or "")
        place_id = place_info["place_id"]
        place_name = place_info.get("name") or shared_place_name
    ...
    # 3) 리뷰 수집 (Phase 1에서 실측 확정 후 구현 — 현재 NotImplementedError)
    review_list = naver_review_collector.fetch_reviews(
        place_id, config.REVIEW_FETCH_LIMIT
    )

    # 4) Claude 분석 (Phase 2에서 구현 — review_analyst 모듈 예정)
    summary_json = _analyze_reviews(place_name or "", review_list)

    # 5) 포맷 → 발송 (분석 결과 포맷 함수는 Phase 2에서 구현)
    message = _format_summary(place_name or "", address or "", summary_json)
...
def _analyze_reviews(place_name: str, review_list: list[dict]) -> str:
    """리뷰 리스트를 Claude로 분석해 PRD §4 JSON 문자열을 반환한다.
...
def _format_summary(place_name: str, address: str, summary_json: str) -> str:
