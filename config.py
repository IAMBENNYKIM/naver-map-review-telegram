...
# ---------------------------------------------------------------------------
# Claude API 리뷰 요약 (review_analyst)
# ---------------------------------------------------------------------------
ANTHROPIC_MODEL: str = "claude-sonnet-4-6"   # 요약 생성 모델 (변경 시 이 상수만 수정)
LLM_COMMENTARY_ENABLED: bool = True          # 요약 기능 킬 스위치 (False면 호출 생략)
LLM_MAX_OUTPUT_TOKENS: int = 2000            # 요약 응답 max_tokens

# ---------------------------------------------------------------------------
# 네이버 지도 리뷰 수집기 (naver_review_collector)
# ---------------------------------------------------------------------------
# 네이버 지도(place)는 iframe·비공식 JSON API 구조 — 실제 엔드포인트/셀렉터는 봇 기동 시 확정.
...
REVIEW_FETCH_LIMIT: int = 20                 # 요약에 사용할 최대 리뷰 수
...
REVIEW_CACHE_TTL: int = 6 * 60 * 60          # 캐시 TTL(초) — 6시간
...
# Claude API 키 (요약 기능 — 없으면 review_analyst가 None 반환해 요약만 생략)
