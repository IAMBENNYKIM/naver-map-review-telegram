...
10. **네이버 스크래핑 함정**: ① `Referer: https://map.naver.com/` 헤더 없으면 응답 0건 가능 ② 인코딩 CP949 가능성 — `response.encoding` 확인 ③ 엔드포인트·파라미터는 반드시 실측 덤프(`experiments/dumps/`)로 확정 후 하드코딩. 요청 간 `RATE_LIMIT_DELAY`(0.5초) 준수.
