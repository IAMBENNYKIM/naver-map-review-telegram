...
- [x] 5-12 (A/D) 후속 UI 개선 3종 (2026-07-21~22):
  - **모바일 줄바꿈 수정(#1)**: 좁은 화면에서 상단 탭·로그아웃 라벨이 줄바꿈되던 문제를 수정(줄바꿈 방지). 커밋 `c760eb8`.
  - **검색 결과 접기 토글(#2)**: 검색 후보의 인라인 분석 결과를 항목 재클릭으로 접고 펼친다(배치·다중 링크 결과가 길어질 때 목록 가독성 유지). 커밋 `c7c7800`.
  - **보관함 접기 토글 + 보관함 내 검색(#3)**: 보관함(`HistoryView`)에서 완료 항목 재클릭 접기 토글 + 식당명·주소 로컬 필터(서버 호출 없는 클라이언트 필터). 커밋 `717bca1`.
  - 프론트 전용 개선(백엔드·인프라 무변경). 계약은 5-11의 `/history`·검색 배치 흐름 재사용.
