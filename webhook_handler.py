"""인바운드 Webhook Lambda 핸들러 (진입점).

API Gateway로 들어온 Telegram update를 검증·파싱해 command_router로 전달하는 진입점.

필수 패턴: `async def lambda_handler` 금지 → 동기 래퍼에서 asyncio.run 호출.
3초 응답(옵션 A): command_router.route를 동기 실행한 뒤 200을 반환한다. 처리 중 예외가
나도 200으로 흡수해 Telegram의 재시도 폭주(중복 발송)를 막는다.
...
async def _async_main(event, context) -> dict:
    """update 검증·파싱 후 command_router.route 동기 실행. 항상(예외 포함) 200 반환."""
    ...
    # 5) 라우팅(옵션 A: 동기 처리). 예외도 흡수해 항상 200 — Telegram 재시도 폭주 방지.
    try:
        command_router.route(text, chat_id)
    except Exception as error:  # noqa: BLE001
        logger.error("command_router.route 처리 실패(chat_id=%s): %s", chat_id, error)
